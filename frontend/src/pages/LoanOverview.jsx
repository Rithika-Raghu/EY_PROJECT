...
const LoanOverview = () => {
  ...
  if (error && !loanData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0A0E12] via-[#0D1216] to-black text-gray-200 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-red-500/10 border border-red-500/30 rounded-2xl p-8 text-center">
          ...
          <h2 className="text-xl font-bold mb-2">Unable to Load Loan Data</h2>
          <p className="text-sm opacity-80 mb-4">{error}</p>
          <button
            onClick={() => window.history.back()}
            className="px-6 py-2 bg-emerald-500 hover:bg-emerald-600 rounded-xl transition-all"
          >
            Go Back
          </button>
        </div>
  ...
  // Payment schedule (this should also come from API)
  const upcomingPayments = [
    { date: loanData.nextDueDate, amount: loanData.monthlyEMI, status: "Upcoming" },
    { date: getNextMonthDate(loanData.nextDueDate, 1), amount: loanData.monthlyEMI, status: "Scheduled" },
    { date: getNextMonthDate(loanData.nextDueDate, 2), amount: loanData.monthlyEMI, status: "Scheduled" },
  ];

  // Helper function to calculate next month dates
  function getNextMonthDate(dateStr, monthsToAdd) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A0E12] via-[#0D1216] to-black text-gray-200">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm opacity-70">
                  Account: {loanData.loanAccountNumber} • {user?.username}
                </p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-500/20 to-blue-500/20 backdrop-blur-xl p-5 rounded-2xl border border-purple-500/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-lg">Welcome, {user?.username || 'User'}!</h3>
        ...
        <div className="bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 backdrop-blur-xl p-5 rounded-2xl border border-emerald-500/30 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-lg">Loan Status: {loanData.status} & In Good Standing</h3>
            <p className="text-sm opacity-80">
